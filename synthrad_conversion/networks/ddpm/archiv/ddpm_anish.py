...
class DiffusionAnish: #(nn.Module)
    def __init__(self,config,my_paths):
        ...
        self.model = DiffusionModelUNet(
            ...
            in_channels=2,
            out_channels=1,
            num_channels= config.ddpm.num_channels,  # (128, 256, 256), (32, 64, 64, 64) (64, 128, 256, 256)
            norm_num_groups=config.ddpm.norm_num_groups,
            attention_levels= config.ddpm.attention_levels,
            num_res_blocks= config.ddpm.num_res_units, # 2
            num_head_channels=config.ddpm.num_head_channels, # 256
        )
    ...
    def _test_nifti(self, val_loader, load_pretrained=True):
        ...
        for step, batch in enumerate(val_loader): 
            if step >= self.config.train.sample_range_lower and step <= self.config.train.sample_range_upper: 
                ...
                val_loss, input_image, orig_image, generated_image = self._sample(model=model, 
                                                                    targets=targets, 
                                                                    inputs=inputs, 
                                                                    inferer=inferer,
                                                                    scheduler=scheduler,
                ...
                loss_tracker.update(val_loss.item())
                ...
                printout=(
                    "[Epoch %d] [Batch %d] [loss: %f] [ssim: %f] [psnr: %f] [mae: %f] \n"
                    % (
                        epoch,
                        step,
                        val_loss.item(),
