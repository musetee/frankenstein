...
def load_nii_and_save_png(nii_file_path, output_png_path, output_fov_png_path, slice_index, dpi=300, 
    ...
    slice_data = pad_image(slice_data, desired_size=[512,512])
...
patient_ID_list = ['9', '10']
slice_list = [30, 47]
FOV_list = [[256,256,100], [256,256,100]]
...
for subdir in sorted(Path(root_dir).iterdir()):  # Iterate through numbered subdirectories
    if subdir.is_dir() and os.path.basename(subdir) in patient_ID_list:
        ...
        for file in sorted(subdir.iterdir()):
            if file.suffix == ".nrrd":
                if "_synthesis" in file.name:
                    synth_file = file
                elif "_mask" in file.name:
                    mask_file = file
                elif "_mask" not in file.name:
                    target_file = file
        
        # If both target and synthesized files are found, process them
        if target_file and synth_file and mask_file:
            print(f"Save image: Target={target_file.name}, Mask={mask_file.name}, Synthesized={synth_file.name}")
            
            slice_ID = slice_list[index]
            FOV_info = FOV_list[index]
            center_x = FOV_info[0]
            center_y = FOV_info[1]
            fov_size = FOV_info[2]

            slice_folder = subdir
            target_image_name = target_file.name
            ...
            synthsized_image_name = synth_file.name
...
for item in folder_list:
    ...
    for slice_item in pID_slice_list:
        ...
        target_image_path = os.path.join(slice_folder, target_image_name)
        target_output_filename = f"{model_name}_{patient_ID}_target_volume.png"
        target_output_path = os.path.join(output_folder, target_output_filename)
        
        seg_image_name = f"{patient_ID}_seg_volume.nii.gz"
        seg_image_path = os.path.join(slice_folder, seg_image_name) 
        seg_output_filename = f"{model_name}_{patient_ID}_seg_volume.png"
        seg_output_path = os.path.join(output_folder, seg_output_filename)    
        
        synthsized_image_name = f"{patient_ID}_synthesized_volume.nii.gz"
        synthsized_image_path = os.path.join(slice_folder, synthsized_image_name)
        synthsized_output_filename = f"{model_name}_{patient_ID}_synthsized_volume.png"
        synthesized_output_path = os.path.join(output_folder, synthsized_output_filename)
        if not only_save_fov:
            fov_box=True
            load_nii_and_save_png(target_image_path, target_output_path, slice_ID, fov_box=fov_box, center_x=center_x, center_y=center_y, fov_size=fov_size)
            load_nii_and_save_png(seg_image_path, seg_output_path,slice_ID, fov_box=fov_box, center_x=center_x, center_y=center_y, fov_size=fov_size)
            load_nii_and_save_png(synthsized_image_path, synthesized_output_path, slice_ID, fov_box=fov_box, center_x=center_x, center_y=center_y, fov_size=fov_size)
        
        nii_file_path = synthsized_image_path
        output_png_filename = f"{model_name}_{patient_ID}_fov_synthsized_volume.png"
        output_png_path = os.path.join(output_folder, output_png_filename)
        
        gt_nii_file_path = target_image_path
        gt_output_png_filename = f"{model_name}_{patient_ID}_fov_target_volume.png"
        gt_output_png_path = os.path.join(output_folder, gt_output_png_filename)

        seg_nii_file_path = seg_image_path
        seg_output_png_filename = f"{model_name}_{patient_ID}_fov_seg_volume.png"
        seg_output_png_path = os.path.join(output_folder, seg_output_png_filename)
