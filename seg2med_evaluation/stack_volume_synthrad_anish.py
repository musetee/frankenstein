...
def extract_patient_and_slice_info(file_path):
    ...
    parts = filename.split('_')
    patient_id = f'{parts[0]}_{parts[1]}'  # Assuming the patient ID format like '1PC082'
...
def stack_volume(folder_path):
    ...
    print(len(seg_paths), len(synthesized_paths), len(target_paths))
    stack_volume_for_one_type(seg_paths, folder_path)
    stack_volume_for_one_type(synthesized_paths, folder_path)
    stack_volume_for_one_type(target_paths, folder_path)

def stack_volume_for_one_type(file_paths,folder_path):
...
if __name__ == "__main__":
    mode = 'for_single' # 'for_single'
    if mode == 'for_single':
        # Set the path to the folder containing your slices
        folder_path = r'D:\Project\seg2med_Project\synthrad_results\results_all_eval\phase3_ctmr\20241226_1148_Infer_ddpm2d_seg2med_ct2mr_512\saved_outputs'
        ...
        root_path = r'D:\Project\seg2med_Project\synthrad_results\results_all_eval\1128results\1128anish'
        folder_list = [
            '20241128_1342_Infer_pix2pix_anish',
            '20241128_1354_Infer_AttentionUnet_anish',
                    ]
